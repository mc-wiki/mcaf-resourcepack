...
crowdin_project_id = os.getenv("CROWDIN_PROJECT_ID")
crowdin_personal_token = os.getenv("CROWDIN_PERSONAL_TOKEN")
assert crowdin_project_id, "Missing environment variable CROWDIN_PROJECT_ID"
assert crowdin_personal_token, "Missing environment variable CROWDIN_PERSONAL_TOKEN"

# Get locale ID map
script_dir = Path(__file__).parent
base_dir = script_dir.parent
crowdin_config = yaml.safe_load(base_dir.joinpath("crowdin.yml").open())
locale_map = crowdin_config["languages_mapping"][0]["locale"]

# Get current translation progress from Crowdin
crowdin_api_url = (
    f"https://crowdin.com/api/v2/projects/{crowdin_project_id}/languages/progress"
)
crowdin_api_result = requests.get(
    crowdin_api_url,
    headers={
...
for lang in crowdin_api_result:
    ...
    for readme_file, readme_file_content in readme_files.items():
        readme_files[readme_file] = re.sub(
